...
function getAverage(value1, value2) {
  const avgTwoNumbers = (value1 + value2) / 2;
  if (avgTwoNumbers === Infinity) {
    return Number.MAX_VALUE;
  }
  return avgTwoNumbers;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceBetweenPoints = Math.hypot(x1 - x2, y1 - y2);
...
function parseNumberFromString(value) {
  const parseNumber = Number.parseFloat(value);
  return parseNumber;
}
